...
const (
	READ_LEN = Dm_build_397
)

type iOffRowBinder interface {
	read(buf *Dm_build_1201)
	isReadOver() bool
	getObj() interface{}
}
...
func newOffRowBytesBinder(obj []byte, encoding string) *offRowBytesBinder {
	...
	binder.offRow = binder.buffer.Dm_build_1206() > Dm_build_394
	return binder
}

func (b *offRowBytesBinder) read(buf *Dm_build_1201) {
	if b.buffer.Dm_build_1206() > 0 {
		buf.Dm_build_1238(b.buffer)
	} else if !b.readOver {
		var obj = b.obj.([]byte)
		buf.Dm_build_1227(obj, 0, len(obj))
		b.readOver = true
	}
}
...
func (b *offRowBlobBinder) read(buf *Dm_build_1201) {
	if b.buffer.Dm_build_1206() > 0 {
		buf.Dm_build_1238(b.buffer)
	} else if !b.readOver {
		...
		buf.Dm_build_1227(bytes, 0, len(bytes))
	}
}
...
func (b *offRowClobBinder) read(buf *Dm_build_1201) {
	if b.buffer.Dm_build_1206() > 0 {
		buf.Dm_build_1238(b.buffer)
	} else if !b.readOver {
		...
		var bytes = Dm_build_920.Dm_build_1136(str, b.encoding, nil)
		b.position += readLen
		if b.position == int32(totalLen) {
			b.readOver = true
		}
...
func (b *offRowReaderBinder) read(buf *Dm_build_1201) {
	if b.buffer.Dm_build_1206() > 0 {
		buf.Dm_build_1238(b.buffer)
	} else if !b.readOver {
		var err error
		...
		buf.Dm_build_1227(bytes[0:readLen], 0, readLen)
	}
}

func (b *offRowReaderBinder) readAll() []byte {
	var byteArray = Dm_build_1205()
	b.read(byteArray)
	for !b.readOver {
		b.read(byteArray)
	}
	return byteArray.Dm_build_1248()
}
