...
func dmdtDecodeFast(value []byte) []int {
	...
	if dtype == DATE {
		...
		dt[OFFSET_YEAR] = int(Dm_build_920.Dm_build_1017(value, 0)) & 0x7FFF
		...
		dt[OFFSET_NANOSECOND] = ((int(value[2]) >> 1) & 0x7f) + ((int(value[3]) & 0x00ff) << 7) + ((int(value[4]) & 0x1F) << 15)
		dt[OFFSET_NANOSECOND] *= 1000
		dt[OFFSET_TIMEZONE] = int(Dm_build_920.Dm_build_1017(value, 5))
		...
		dt[OFFSET_TIMEZONE] = int(Dm_build_920.Dm_build_1017(value, len(value)-2))
...
func dmdtDecodeBdta(value []byte) []int {
	dt := make([]int, DT_LEN)
	dt[OFFSET_YEAR] = int(Dm_build_920.Dm_build_1017(value, 0))
	...
	dt[OFFSET_TIMEZONE] = int(Dm_build_920.Dm_build_1017(value, 10))
...
func encode(dt []int, column column, lTz int, dTz int) ([]byte, error) {
	...
	if column.colType == DATE {
		...
		Dm_build_920.Dm_build_931(ret, 8, int16(tz))
		...
		Dm_build_920.Dm_build_931(ret, 5, int16(tz))
	}
